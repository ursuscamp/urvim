...
pub use urvim_lsp::config::InlayHintCapability;
...
pub use urvim_lsp::config::LspConfig;

/// The resolved configuration for a single LSP server.
pub use urvim_lsp::config::LspServerConfig;
...
fn resolve_lsp_server(
    ...
    let merged_toml = merge_toml_values(
        serde_json_to_toml_value(builtin.settings.clone()),
        file.and_then(|config| config.settings.as_ref()),
    );
    let settings = serde_json::to_value(&merged_toml).unwrap_or(serde_json::Value::Null);
...
fn serde_json_to_toml_value(json: serde_json::Value) -> Value {
    serde_json::from_value(json).unwrap_or(Value::Table(TomlMap::new()))
}
