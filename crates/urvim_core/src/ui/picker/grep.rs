...
mod tests {
    ...
    fn query_prompt_segments_include_mode_label() {
        let theme = urvim_theme::Theme::new(
            "prompt-test",
            urvim_theme::ThemeKind::Ansi256,
            Style::default(),
            urvim_theme::HighlightStyles::new(
                [
                    (
                        "ui.input.prompt.exact",
                        Style::new().fg(urvim_terminal::Color::ansi(1)).bold(),
                    ),
                    (
                        "ui.input.prompt.fuzzy",
                        Style::new().fg(urvim_terminal::Color::ansi(2)).italic(),
                    ),
                    (
                        "ui.input.prompt.separator",
                        Style::new().fg(urvim_terminal::Color::ansi(3)).faint(),
                    ),
                ]
                .into_iter()
                .map(|(name, style)| (urvim_theme::Tag::parse(name).expect("valid tag"), style))
                .collect(),
            ),
        );
