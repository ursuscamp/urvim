...
mod tests {
    ...
    fn doc_symbols_picker_item_uses_syntax_style_for_symbol_kind() {
        let mut highlights = urvim_theme::HighlightStyles::default();
        highlights.insert(
            urvim_theme::Tag::parse("syntax.function").expect("valid tag"),
            Style::new().fg(urvim_terminal::Color::ansi(42)).bold(),
        );
        highlights.insert(
            urvim_theme::Tag::parse("ui.input.prompt").expect("valid tag"),
            Style::new().fg(urvim_terminal::Color::ansi(20)),
        );
        let _guard = globals::set_test_active_theme(urvim_theme::Theme::new(
            "demo",
            urvim_theme::ThemeKind::Ansi256,
            Style::new()
                .fg(urvim_terminal::Color::ansi(9))
                .bg(urvim_terminal::Color::ansi(1)),
            highlights,
        ));
        let _config_guard = globals::set_test_config(Config {
            advanced_glyphs: BTreeSet::from([crate::config::AdvancedGlyphCapability::Nerdfont]),
        ...
        let segments = item.render_segments(24, Style::new().bg(urvim_terminal::Color::ansi(12)));
        assert_eq!(
            segments[0].style.foreground(),
            Some(urvim_terminal::Color::ansi(20))
        );
        assert_eq!(
            segments[2].style,
            Style::new()
                .bg(urvim_terminal::Color::ansi(12))
                .fg(urvim_terminal::Color::ansi(42))
                .bold()
        );
    }
