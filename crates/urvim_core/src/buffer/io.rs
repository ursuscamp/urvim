...
impl Buffer {
    ...
    pub fn new() -> Self {
        ...
        let syntax_name = SmolStr::new(urvim_syntax::fallback_syntax_name());
    ...
    pub fn new_from_str(text: &str) -> Self {
        ...
        let syntax_name = urvim_syntax::resolve_builtin_syntax(
            None,
            first_line.as_ref().and_then(|line| line.contiguous_text()),
        )
        .unwrap_or_else(|| SmolStr::new(urvim_syntax::fallback_syntax_name()));
        let saved_lines = lines.clone();
        let undo_lines = lines.clone();
        let markers = MarkersStore::with_line_count(lines.line_count());
        Self {
