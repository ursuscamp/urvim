...
/// Structural edit applied to a diff cache.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct DiffEdit {
    /// First line affected by the edit.
    pub start_line: usize,
    /// Net line-count change caused by the edit.
    pub line_delta: isize,
}

impl DiffEdit {
    /// Creates a structural diff-cache edit.
...
impl DiffCache {
    ...
    pub fn apply_edit(&mut self, edit: DiffEdit) {
        self.apply_edits(&[edit]);
    }

    /// Applies structural edits to the cache.
    pub fn apply_edits(&mut self, edits: &[DiffEdit]) {
        if edits.is_empty() {
            return;
        }
