//! Builtin handwritten scanner for OCaml syntax.
...
tag_static!(COMMENT, "comment.block");
...
const OCAML_COMMENT: ContextId = ContextId::new("ocaml", "ocaml_comment");
const OCAML_STRING: ContextId = ContextId::new("ocaml", "ocaml_string");

/// Tokenize one line of OCaml using the builtin scanner.
pub(crate) fn tokenize_ocaml_line(
    line: &str,
    state: SyntaxState,
    ...
    while index < line_len {
        ...
        // ── Inside comment ───────────────────────────────────────────
        if ctx.top_is(OCAML_COMMENT) {
            // Rule 1: *) close
            if tail_bytes.len() >= 2 && tail_bytes[0] == b'*' && tail_bytes[1] == b')' {
                let end = index + 2;
                spans.push(SyntaxSpan::new(index, end, (*COMMENT).clone()));
                ctx.pop_top(OCAML_COMMENT);
                index = end;
                continue;
            }
            // Rule 2: (* nested open (inside comment)
            if tail_bytes.len() >= 2 && tail_bytes[0] == b'(' && tail_bytes[1] == b'*' {
                let end = index + 2;
                spans.push(SyntaxSpan::new(index, end, (*COMMENT).clone()));
                ctx.push(OCAML_COMMENT);
                index = end;
                continue;
            }
            // Rule 3: Content [^*]+
            ...
            // Fall through to top-level so * falls to operator rules
        }

        // ── Inside string ────────────────────────────────────────────
        if ctx.top_is(OCAML_STRING) {
            // Rule 4: Closing "
            if tail_bytes[0] == b'"' {
                let end = index + 1;
                spans.push(SyntaxSpan::new(index, end, (*S).clone()));
                ctx.pop_top(OCAML_STRING);
                index = end;
                continue;
            }
            // Rule 7: Escape \.
        ...
        // Rule 2: Comment open (* (also handled inside comments for nesting)
        if tail_bytes.len() >= 2 && tail_bytes[0] == b'(' && tail_bytes[1] == b'*' {
            let end = index + 2;
            spans.push(SyntaxSpan::new(index, end, (*COMMENT).clone()));
            ctx.push(OCAML_COMMENT);
        ...
        // Rule 10: Built-in type with word boundaries
        if let Some(typ_len) = match_builtin_type(tail, index, bytes) {
        ...
        // Rule 13: Capitalized type with word boundaries
...
fn match_builtin_type(tail: &str, index: usize, full_bytes: &[u8]) -> Option<usize> {
    if index > 0 && is_word_byte(full_bytes[index - 1]) {
        return None;
    }
