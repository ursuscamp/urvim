...
use crate::state::{CodeState, ContextId, ContextStack, SyntaxLineResult, SyntaxSpan, SyntaxState};
use urvim_theme::Tag;
...
pub(crate) fn tokenize_css_line(line: &str, state: SyntaxState) -> SyntaxLineResult {
    ...
    while index < line_len {
        ...
        if ctx.top_is(CSS_DOUBLE_STRING) {
            ...
            // Rule 11: Content up to a double-string boundary.
        ...
        if ctx.top_is(CSS_SINGLE_STRING) {
            ...
            // Rule 12: Content up to a single-string boundary.
            let run = run_while(tail, |c| c != '\'' && c != '\\');
            if run > 0 {
                spans.push(SyntaxSpan::new(index, index + run, (*S).clone()));
                index += run;
        ...
        if !ctx.top_is(CSS_SINGLE_STRING) && tail_bytes[0] == b'"' {
            let end = index + 1;
            spans.push(SyntaxSpan::new(index, end, (*S).clone()));
        ...
        if !ctx.top_is(CSS_DOUBLE_STRING) && tail_bytes[0] == b'\'' {
            let end = index + 1;
            spans.push(SyntaxSpan::new(index, end, (*S).clone()));
