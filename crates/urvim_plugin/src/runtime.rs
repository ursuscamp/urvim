...
pub enum PluginRuntimeEvent {
    ...
    NotificationReceived {
        ...
        notification: crate::PluginNotification,
    },
...
mod tests {
    use super::*;
    use crate::{MANIFEST_FILE_NAME, PluginNotification, PluginRegistry};
    use crate::{PluginConfigEntry, PluginRequest, PluginResponse, encode_frame};
    use serde_json::json;
    ...
    fn config_for_plugin(
        name: &str,
        path: PathBuf,
        enabled: bool,
    ) -> BTreeMap<String, PluginConfigEntry> {
        BTreeMap::from([(name.to_string(), PluginConfigEntry { enabled, path })])
    }
    ...
    fn process_failure_does_not_remove_manifest_contributions() {
        let manifest = crate::PluginManifest::parse_from_str(
            "test",
            r#"
name = "demo"
version = "0.1.0"
