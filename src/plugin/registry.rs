...
impl LoadedPlugin {
    ...
    pub fn commands(&self) -> &BTreeMap<String, crate::plugin::PluginCommand> {
        &self.manifest.commands
    }

    /// Returns process launch configuration, when this plugin has one.
    pub fn process(&self) -> Option<&crate::plugin::PluginProcess> {
...
impl PluginRegistry {
    ...
    pub fn load_from_config(config: &Config) -> Result<Self, PluginLoadError> {
    ...
    pub fn command(&self, plugin: &str, command: &str) -> Option<&crate::plugin::PluginCommand> {
        self.plugins
            .get(plugin)
...
mod tests {
    ...
    use crate::theme::resolve_theme_from_str;
    ...
    fn write_manifest(root: &Path, contents: &str) {
        ...
        std::fs::write(root.join(crate::plugin::MANIFEST_FILE_NAME), contents)
            .expect("manifest should be written");
    }

    fn config_with_plugins(plugins: BTreeMap<String, PluginConfig>) -> Config {
        Config {
            plugins,
            ..Config::default()
        }
    ...
    fn enabled(path: PathBuf) -> PluginConfig {
        PluginConfig {
            enabled: true,
            path,
        }
    ...
    fn disabled(path: PathBuf) -> PluginConfig {
        PluginConfig {
            enabled: false,
            path,
        }
...
"#,
    ...
    #[test]
    fn example_plugin_scripts_are_valid_commands() {
        let manifest = PluginManifest::load_from_dir(EXAMPLE_PLUGIN_ROOT)
            .expect("example plugin manifest should load");

        for (name, commands) in &manifest.scripts {
            assert!(!commands.is_empty(), "script {name} should not be empty");
            for command in commands {
                crate::command::validate_script_command(command)
                    .unwrap_or_else(|error| panic!("script {name} command {command:?}: {error}"));
            }
        }
    }
