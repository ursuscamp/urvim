...
impl FiletypePickerSource {
    ...
    pub fn builtin_items() -> Vec<FiletypePickerItem> {
        let mut items = crate::syntax::builtin_syntax_registry()
            .ok()
            .map(|registry| {
                registry
                    .names()
    ...
    fn filetype_picker_renders_icon_and_muted_name() {
        let item = FiletypePickerItem {
            ...
            icon: Some(FiletypeIcon {
                glyph: "".into(),
                style: Style::new().fg(crate::terminal::Color::ansi(160)),
            }),
        };

        let base_style = Style::new().bg(crate::terminal::Color::ansi(30));
        let segments = item.formatted_line(base_style).render_segments(80);
        assert_eq!(segments.len(), 3);
        assert_eq!(segments[0].text, "  ");
        assert_eq!(
            segments[0].style,
            base_style.accent(Style::new().fg(crate::terminal::Color::ansi(160)))
        );
