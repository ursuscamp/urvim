//! Builtin handwritten scanner for Scala syntax.
...
const SCALA_BLOCK_COMMENT: ContextId = ContextId::new("scala", "scala_block_comment");
const SCALA_TRIPLE_STRING: ContextId = ContextId::new("scala", "scala_triple_string");
const SCALA_STRING: ContextId = ContextId::new("scala", "scala_string");

/// Tokenize one line of Scala using the builtin scanner.
pub(crate) fn tokenize_scala_line(
    line: &str,
    state: SyntaxState,
    ...
    while index < line_len {
        ...
        if ctx.top_is(SCALA_BLOCK_COMMENT) {
            ...
            if tail_bytes.len() >= 2 && tail_bytes[0] == b'/' && tail_bytes[1] == b'*' {
                let end = index + 2;
                spans.push(SyntaxSpan::new(index, end, (*COMMENT_BLOCK).clone()));
                ctx.push(SCALA_BLOCK_COMMENT);
                index = end;
                continue;
            }
        }

        if ctx.top_is(SCALA_TRIPLE_STRING) {
            if tail_bytes.len() >= 3
                && tail_bytes[0] == b'"'
                && tail_bytes[1] == b'"'
                && tail_bytes[2] == b'"'
            {
                ...
                ctx.pop_top(SCALA_TRIPLE_STRING);
                index = end;
                continue;
            }
        ...
        if ctx.top_is(SCALA_STRING) {
            if tail_bytes[0] == b'"' {
                let end = index + 1;
                spans.push(SyntaxSpan::new(index, end, (*S).clone()));
                ctx.pop_top(SCALA_STRING);
                index = end;
                continue;
            }
...
fn match_type(tail: &str, index: usize, full_bytes: &[u8]) -> Option<usize> {
    ...
    for typ in &[
        ...
        "Any", "Option", "Either", "List", "Map", "Set", "Seq", "Array", "Future", "Try",
    ] {
...
fn match_number(tail: &str, index: usize, full_bytes: &[u8]) -> Option<usize> {
    ...
    if i < len && matches!(bytes[i], b'l' | b'L' | b'f' | b'F' | b'd' | b'D') {
        i += 1;
    }
...
fn match_based_number(
    ...
    let mut end = consume_digits_and_underscores(bytes, start, is_valid_digit)?;
    if end < bytes.len() && matches!(bytes[end], b'l' | b'L') {
        end += 1;
    }
