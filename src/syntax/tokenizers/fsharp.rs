//! Builtin handwritten scanner for F# syntax.
...
const FSHARP_BLOCK_COMMENT: ContextId = ContextId::new("fsharp", "fsharp_block_comment");
const FSHARP_TRIPLE_STRING: ContextId = ContextId::new("fsharp", "fsharp_triple_string");
const FSHARP_STRING: ContextId = ContextId::new("fsharp", "fsharp_string");

/// Tokenize one line of F# using the builtin scanner.
pub(crate) fn tokenize_fsharp_line(
    line: &str,
    state: SyntaxState,
    ...
    while index < line_len {
        ...
        if ctx.top_is(FSHARP_TRIPLE_STRING) {
            // Rule 4: Closing """
            if tail_bytes.len() >= 3
                && tail_bytes[0] == b'"'
                && tail_bytes[1] == b'"'
                && tail_bytes[2] == b'"'
            {
                ...
                ctx.pop_top(FSHARP_TRIPLE_STRING);
                index = end;
                continue;
            }
        ...
        if ctx.top_is(FSHARP_STRING) {
            // Rule 8: Closing "
            if tail_bytes[0] == b'"' {
                let end = index + 1;
                spans.push(SyntaxSpan::new(index, end, (*S).clone()));
                ctx.pop_top(FSHARP_STRING);
                index = end;
                continue;
            }
            // Rule 11: Escape \.
            ...
            && tail_bytes[2] == b'"'
        {
            let end = index + 3;
            spans.push(SyntaxSpan::new(index, end, (*S).clone()));
            ctx.push(FSHARP_TRIPLE_STRING);
            index = end;
            continue;
        }

        // Rule 7: Verbatim string @"..."
        ...
        // Rule 13: Number with word boundaries (checked before keyword
        //          since numbers come first in TOML order)
...
fn match_number(tail: &str, index: usize, full_bytes: &[u8]) -> Option<usize> {
    ...
    // Decimal: \d[\d_]*(?:\.\d[\d_]*)?(?:[eE][+-]?\d[\d_]*)?[fFmMlL]?
    ...
    if i < len && matches!(bytes[i], b'f' | b'F' | b'm' | b'M' | b'l' | b'L') {
        i += 1;
    }
