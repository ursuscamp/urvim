...
use crate::buffer::syntax::{
    CodeState, ContextId, ContextStack, SyntaxFoldEvent, SyntaxFoldEventKind, SyntaxLineResult,
    SyntaxSpan, SyntaxState, tokenize_injected_body,
};
use crate::theme::Tag;
...
const RUST_FOLD_BRACKET: u32 = 2;

pub(crate) fn tokenize_rust_line(line: &str, state: SyntaxState) -> SyntaxLineResult {
    ...
    while index < line.len() {
        ...
        if ctx.top_is(FORMAT_CALL) && tail.starts_with('(') {
            let end = index + 1;
            push_rust_fold_event(&mut fold_events, index, tail, &ctx);
        ...
        if tail.starts_with("#![") || tail.starts_with("#[") {
            ...
            let end = index + len;
            push_rust_fold_event_for_delimiter(
                &mut fold_events,
...
fn push_rust_fold_event(
    fold_events: &mut Vec<SyntaxFoldEvent>,
    _byte: usize,
    tail: &str,
    ctx: &ContextStack,
) {
    if ctx.top_is(RUST_STRING)
        || ctx.top_is(RUST_RAW_STRING_BODY)
        || ctx.top_is(RUST_BYTE_CHAR)
        || ctx.top_is(RUST_DOC_COMMENT)
        || ctx.top_is(RUST_BLOCK_COMMENT)
        || ctx.top_is(FORMAT_STRING)
        || ctx.top_is(FORMAT_EXPR)
    {
        return;
    ...
    super::bracket_folds::push_delimiter_fold_event(fold_events, tail.as_bytes()[0]);
}

fn push_rust_fold_event_for_delimiter(
    fold_events: &mut Vec<SyntaxFoldEvent>,
    kind: SyntaxFoldEventKind,
    fold_kind: u32,
) {
